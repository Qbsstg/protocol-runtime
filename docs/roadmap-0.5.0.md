...
| --- | --- | --- |
| `runtime-ingress-http` | Accept HTTP POST requests, map request bodies to `IngressEnvelope`, choose response policy from runtime outcome. | Uses JDK `HttpServer` for the first baseline; must not depend on protocol SDK modules or third-party HTTP frameworks. |
...
4. Choose one narrow adapter baseline for implementation after the boundary is
   reviewed. HTTP ingress is the first baseline because it can be tested with
   JDK APIs and does not add broker dependencies.
5. Add repository-only smoke tests for any implemented adapter path. The HTTP
   baseline includes local `HttpServer` tests for source mapping, request
   limits, response policy, and backpressure decisions.
...
also avoids tag creation or real Maven Central upload. `0.5.0` has since been
tagged as `v0.5.0`, published to Maven Central, verified with an isolated local
Maven repository, and published as a GitHub Release.
