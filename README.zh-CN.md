...
诊断流程。`0.17.0` 发布第一轮 downstream sink 生产化 baseline：file sink schema v1、
投递失败分类、失败记录隔离、有界失败样本导出、sink backpressure 证据、重试/死信边界
设计、Kafka/HTTP/MQTT downstream sink adapter 边界、record envelope 输出规范、
operator sink troubleshooting 和 smoke 覆盖。

当前 `0.17.0` baseline 范围记录在
...
最新 release candidate 运行时版本是 `0.17.0`。Runtime 模块是 JDK 21 artifact。应用侧应按需
直接依赖具体模块：
