...
`0.10.0` 已发布健康检查和运行状态生产化能力，包括 app-local
...
文档和 smoke 覆盖。`0.14.0` 已发布 standalone collector 的第一轮运行包分发治理
baseline，包括 zip/tar 分发包、`bin`/`conf`/`logs`/`data`/`run`/`tmp` 目录模板、
...
诊断流程。`0.17.0` 已发布第一轮 downstream sink 生产化 baseline：file sink schema v1、
投递失败分类、失败记录隔离、有界失败样本导出、sink backpressure 证据、重试/死信边界
设计、Kafka/HTTP/MQTT downstream sink adapter 边界、record envelope 输出规范、
operator sink troubleshooting 和 smoke 覆盖。`0.18.0` release 分支发布 downstream
sink adapter SPI、record envelope contract、delivery result contract、retry/dead-letter 边界、
Kafka/HTTP/MQTT sink adapter 模块边界、adapter 配置模型、adapter smoke、operator
troubleshooting 和依赖边界策略。

当前 `0.18.0` downstream sink adapter SPI baseline 范围记录在
[`docs/roadmap-0.18.0.md`](docs/roadmap-0.18.0.md)，release notes 记录在
[`docs/release-notes-0.18.0.md`](docs/release-notes-0.18.0.md)，release-readiness
audit 记录在 [`docs/release-readiness-0.18.0.md`](docs/release-readiness-0.18.0.md)。已发布的 `0.17.0`
范围记录在
[`docs/roadmap-0.17.0.md`](docs/roadmap-0.17.0.md)，release notes 记录在
[`docs/release-notes-0.17.0.md`](docs/release-notes-0.17.0.md)，release-readiness
audit 记录在 [`docs/release-readiness-0.17.0.md`](docs/release-readiness-0.17.0.md)。已发布的 `0.16.0`
范围记录在
...
[`docs/release-readiness-0.14.0.md`](docs/release-readiness-0.14.0.md)，运行包安装和升级说明记录在
[`docs/distribution-package.zh-CN.md`](docs/distribution-package.zh-CN.md)。已发布的 `0.13.0`
...
最新已发布运行时版本是 `0.17.0`。Runtime 模块是 JDK 21 artifact。应用侧应按需
直接依赖具体模块：
...
| `runtime-app` | 0.17.0 baseline | Standalone collector 装配层，支持 properties 配置、app 级协议选择、TCP/HTTP/Kafka/MQTT 装配、JDK logging/file/in-memory sink、稳定 file sink JSONL schema、sink 投递失败分类、失败记录隔离、有界失败样本导出、file sink 状态、sink-failure-triggered backpressure、app-local health/readiness 快照、可解释状态输出、JDK HTTP 管理端点、管理端访问控制、请求日志、管理端 metrics、bounded health history、统一错误 JSON、可执行 shaded jar、app-owned 生产部署治理、zip/tar 运行包分发治理、包内元数据、版本诊断、包完整性校验、runtime self-check、配置 hot-check 但不 hot-reload、checksum sidecar、长期运行 smoke、release artifact smoke/回归 smoke、sink failure smoke 和运维排障/runbook。 |
...
`0.18.0` 从已发布的 `0.17.0` downstream sink 生产化版本继续推进。这个
...
## `0.17.0` Downstream Sink 生产化 Baseline

`0.17.0` 从已发布的 `0.16.0` 生产运行增强版本继续推进。这个 baseline
先加固 standalone collector 当前 app-owned file/logging/in-memory sink 路径，再考虑
后续是否引入专门的 broker 或 HTTP producer 依赖。

第一轮 baseline 范围包括：

- file sink schema v1 和 record envelope 输出规范
- 配置、文件系统、序列化、写入、flush、背压、可重试临时失败、永久拒绝、未知失败等投递失败分类
- app-local 失败记录隔离和有界失败样本导出
- file sink failure、失败隔离失败和未来 broker sink 场景下的 sink backpressure 状态/readiness 证据
- 不引入持久化存储的重试和死信边界设计
- Kafka、HTTP、MQTT downstream sink adapter 的未来 `runtime-sink-*` 模块边界
- operator sink troubleshooting 和 smoke 覆盖预期

该 baseline 不把 Kafka producer、HTTP client、MQTT publisher、数据库写入、Redis 队列、
对象存储、外部队列或 sink adapter 依赖引入 `runtime-core`。

## `0.16.0` 生产运行增强 Baseline

`0.16.0` 从已发布的 `0.15.0` 运行包生产化版本继续推进，目标是让 standalone
collector 更适合长期生产运行。该 baseline 仍保持生产运行能力只在 app/docs/smoke
边界内演进，不引入 runtime
supervisor、service manager、数据库、Redis、外部观测 exporter 或应用框架依赖。

第一轮 baseline 范围包括：

- `bin/protocol-runtime self-check`：输出配置、Java 版本、运行包布局、可写目录、sink
  路径、listener bind 准备状态、management endpoint 姿态和 package integrity 状态的
  JSON 证据。
- `bin/protocol-runtime hot-check`：检测配置文件变化、重新校验配置，并报告是否需要重启；
  该命令不会对运行中的 collector 做热加载。
- 启动、停止、listener bind、sink health、解析失败、背压决策、management request 和运行包完整性状态的日志与状态证据增强。
- stale PID、端口冲突、sink 路径失败、配置错误、management token 错误、解析失败、
  backpressure、运行包校验失败和升级中断等场景的失败恢复手册。
- 长期运行 smoke 和 release artifact 回归 smoke。
- operator runbook 和生产问题诊断流程，指导收集 version、配置校验、自检、状态导出、
  日志、PID、management snapshot 和运行包校验证据。

运维细节维护在 [`docs/operations-runbook.zh-CN.md`](docs/operations-runbook.zh-CN.md)。

相关工作只能放在 `runtime-app`、examples、docs、CI/smoke 或未来 dedicated
app/operations 边界中，不允许把 runtime-supervisor、service-manager、外部 exporter、
数据库、Redis、Spring、installer、package manager 或反向 SDK 依赖引入核心模块。

## `0.13.0` 生产部署治理 baseline
...
- smoke 覆盖验证配置校验、dry-run、PID 文件创建、状态导出、management HTTP 检查
  和 standalone jar 优雅停止。

详细规划维护在 [`docs/roadmap-0.13.0.md`](docs/roadmap-0.13.0.md)，部署指南维护在
[`docs/deployment-governance.zh-CN.md`](docs/deployment-governance.zh-CN.md)。
...
`0.15.0` 已发布第一轮运行包生产化增强 baseline，包括：
...
service-manager、filesystem-layout、deployment-wrapper、数据库、Redis、框架或 exporter
依赖引入 `runtime-core` 或 `protocol-sdk`。

## `0.14.0` 运行包分发治理 Baseline

`0.14.0` 已为 standalone collector 发布可部署运行包，同时把分发治理边界限制在
`runtime-app`、构建配置、examples 和 docs。tag `v0.14.0` 指向 release commit，
runtime artifacts、standalone classifier 以及 distribution zip/tar.gz 包已经可从
Maven Central 获取。

构建 standalone jar 和运行包：
...
分发包 smoke 会验证解压、JDK 21 检查、配置校验、dry-run、状态导出、启动、
management health/readiness/status、file sink 输出、重复启动、TCP 端口冲突和优雅停止：
...
安装、升级和运维说明维护在
[`docs/distribution-package.zh-CN.md`](docs/distribution-package.zh-CN.md)。
...
- 管理 HTTP 继续独立于 `runtime-ingress-http`；后者仍只负责协议 payload 的
  HTTP 采集接入。
- 在扩展远程控制能力前，先明确管理端安全边界。
- 可配置访问控制、request logging、JSON metrics、健康历史快照和错误响应规范
...
`0.11.0` 已在发布的 `0.10.0` health/readiness 模型基础上，增加第一版
standalone collector 管理面：

- `runtime-core` 继续不引入 Spring、Netty、Kafka、MQTT、HTTP、数据库、Redis
  和 observability exporter 依赖。
- 管理 HTTP 属于 app 边界，在 `runtime-app` 内使用 JDK `HttpServer` 实现。
...
详细发布记录维护在 [`docs/roadmap-0.11.0.md`](docs/roadmap-0.11.0.md)。

## `0.10.0` 健康检查与状态生产化发布

`0.10.0` 已发布 `0.9.0` 之后的健康检查与状态生产化路线：
...
详细发布记录维护在 [`docs/roadmap-0.10.0.md`](docs/roadmap-0.10.0.md)。
...
`runtime-app` 提供 `0.2.0` 引入的可运行采集器边界，并继续保留 TCP/Netty、
JDK HTTP、Kafka、MQTT 和 management endpoint 路径；这些路径接到同一个
app-owned pipeline：
...
同一命令会在 `runtime-app/target/` 下生成当前 release 线的运行包。最终发布后，
...
sink failure smoke 会验证 file sink 失败分类、失败记录隔离和 sink-failure-triggered
readiness 证据：
...
file sink 每行输出一条稳定 JSONL envelope。`0.17.0` 使用
`protocol-runtime.record.v1`、`protocol-runtime.parse-failure.v1` 和
`protocol-runtime.failed-record.v1` 三个 schema version。当前输出文件超过
`collector.sink.file.maxBytes` 前会触发轮转，并保留 `collector.sink.file.maxHistory`
个历史文件。比如输出路径是 `target/runtime-records.ndjson` 时，历史文件命名为
`runtime-records.ndjson.1`、`runtime-records.ndjson.2`，依次类推。collector status
输出会展示当前输出文件路径、file sink 是否 open、当前活跃文件字节数、保留的历史文件
数量、进程内累计轮转次数、轮转限制、当前 schema version、失败记录隔离目录、保留样本数和
最后一次 sink failure 分类。
...
- `sourceId`
- `protocol`
- `receivedAt`，当 ingress adapter 提供接收时间元数据时存在
- `parsedAt`
- `recordType`
- `observedAt`
- `rawPayloadHex`
- `quality.status`
- `payload.value`、`payload.rawHex`、`payload.rawSize`
- `raw.metadata`
- `parser.diagnostics`
- `sink.delivery`
...
`attributes` 和可选 `cause`。当前 app 的 parse failure 策略是 continue：异常帧会进入
配置的 failure sink，但不会停止 collector，也不会阻止后续健康帧解析。

runtime-app 还会在应用装配边界隔离 sink 写入异常。如果 record sink 或 failure
sink 在处理解析结果时抛出异常，该异常会记录到 collector metrics 和 status 输出，
不会反向传播进 `runtime-core` 或 ingress adapter。失败样本会以
`protocol-runtime.failed-record.v1` JSON 写入 `collector.sink.failedRecords.dir`，
包含 `failureType`、异常类型、消息、是否可重试、source id、protocol、原始 payload hex
和安全的记录元数据。
...
- `JDK 21 or newer is required`：包内 `bin/protocol-runtime` 脚本当前使用的默认
  `java` 太旧。请设置 `JAVA_HOME` 或 `JAVA_BIN` 指向 JDK 21+。
- `collector already appears to be running`：启动脚本在 PID 文件中发现了仍在运行的
  进程。请先停止该进程，或使用不同的 runtime dir。
...
- 失败记录没有导出：检查 `collector.sink.failedRecords.enabled`、
  `collector.sink.failedRecords.dir`、目录权限，以及 status JSON 中的
