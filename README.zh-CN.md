...
`0.14.0-SNAPSHOT` 已在 `0.13.0` 发布后打开，并已包含 standalone collector 的第一轮
...
[`docs/release-notes-0.14.0.md`](docs/release-notes-0.14.0.md)，运行包安装和升级说明记录在
...
`0.14.0-SNAPSHOT` 为 standalone collector 增加可部署运行包，同时把分发治理边界限制在
`runtime-app`、构建配置、examples 和 docs。
...
当前开发线中，同一命令也会在 `runtime-app/target/` 下生成 `0.14.0-SNAPSHOT`
运行包。
