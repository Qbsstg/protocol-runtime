...
当前 `0.16.0-SNAPSHOT` 开发线用于规划下一轮生产运行增强：长期运行稳定性、运行时
自检、配置热检查但不热加载、日志与状态证据增强、失败恢复手册、长期运行 smoke、
release artifact 回归 smoke、operator runbook 和生产问题诊断流程。
...
| `runtime-app` | 0.15.0 baseline | Standalone collector 装配层，支持 properties 配置、app 级协议选择、TCP/HTTP/Kafka/MQTT 装配、JDK logging/file/in-memory sink、sink 失败隔离、file sink 状态、sink-failure-triggered backpressure、app-local health/readiness 快照、可解释状态输出、JDK HTTP 管理端点、管理端访问控制、请求日志、管理端 metrics、bounded health history、统一错误 JSON、可执行 shaded jar、app-owned 生产部署治理、zip/tar 运行包分发治理、包内元数据、版本诊断、包完整性校验、checksum sidecar、release artifact smoke 和运维排障。 |
...
collector 更适合长期生产运行。该规划只做生产运行边界设计，不引入 runtime
supervisor、service manager、数据库、Redis、外部观测 exporter 或应用框架依赖。

第一轮规划范围包括：

- 长期运行稳定性要求，以及长时间 smoke 中需要保留的证据。
- 运行时自检输出，覆盖配置、Java 版本、运行包布局、可写目录、sink 路径、listener
  bind 准备状态和 management endpoint 姿态。
- 配置热检查能力：检测配置文件变化并输出校验结果，但不对运行中的 collector 做热加载。
...
规划工作只能放在 `runtime-app`、examples、docs、CI/smoke 或未来 dedicated
