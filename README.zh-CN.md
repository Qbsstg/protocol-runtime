...
当前 `0.17.0-SNAPSHOT` 开发线实现第一轮 downstream sink 生产化 baseline：file
sink schema v1、投递失败分类、失败记录隔离、有界失败样本导出、sink backpressure 证据、
...
| `runtime-app` | 0.16.0 baseline | Standalone collector 装配层，支持 properties 配置、app 级协议选择、TCP/HTTP/Kafka/MQTT 装配、JDK logging/file/in-memory sink、sink 失败隔离、file sink 状态、sink-failure-triggered backpressure、app-local health/readiness 快照、可解释状态输出、JDK HTTP 管理端点、管理端访问控制、请求日志、管理端 metrics、bounded health history、统一错误 JSON、可执行 shaded jar、app-owned 生产部署治理、zip/tar 运行包分发治理、包内元数据、版本诊断、包完整性校验、runtime self-check、配置 hot-check 但不 hot-reload、checksum sidecar、长期运行 smoke、release artifact smoke/回归 smoke 和运维排障/runbook。 |
...
`0.17.0-SNAPSHOT` 从已发布的 `0.16.0` 生产运行增强版本继续推进。这个 baseline
